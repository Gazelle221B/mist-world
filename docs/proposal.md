...
形式は `https://mist.world/join?room={roomId}&token={signedToken}&exp={timestamp}` とする。有効期限のデフォルトは 24 時間で、選択肢として 1 h / 6 h / 24 h / 7 d を提供する。signedToken は作成者の Ed25519 秘密鍵で署名する。失効は作成者がローカルの revocation list に追加し、CRDT メタデータに伝播させることで全ピアが拒否する。
...
.mistworld ファイル生成（version, seed, wasmHash, buildings）を実装する。エクスポートボタンとインポートによるマップ + 建築復元を実装する。クロスブラウザテスト（Chrome → Edge）を行う。
