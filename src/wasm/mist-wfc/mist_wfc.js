 ...
 * @returns {string}
 */
export function generate_preview(seed_hi, seed_lo) {
    let deferred1_0;
    let deferred1_1;
    try {
        const ret = wasm.generate_preview(seed_hi, seed_lo);
        deferred1_0 = ret[0];
        deferred1_1 = ret[1];
        return getStringFromWasm0(ret[0], ret[1]);
    } finally {
...
async function __wbg_init(module_or_path) {
    ...
    if (module_or_path === undefined) {
        module_or_path = new URL('mist_wfc_bg.wasm', import.meta.url);
    }
    const imports = __wbg_get_imports();
